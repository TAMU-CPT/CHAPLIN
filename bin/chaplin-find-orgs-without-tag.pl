...
my @tnkeys = /start end strand timelastmodified/;
...
foreach my $organism(@haves){
	...
	foreach(@fp){
		...
		my $table = "<table class=\"table table-striped\"><thead><tr><th>Key</th><th>Value</th></tr></thead><tbody>";
		my %fi = %{$_};

		$table .= sprintf('<tr><td>%s</td><td>%s</td></tr>', $tn{primary_tag}, $cv{$fi{primary_tag}});

		foreach(@tnkeys){
			$table .= sprintf('<tr><td>%s</td><td>%s</td></tr>', $tn{$_}, $fi{$_});
		}
		foreach(@{$fi{featprops}}){
			my ($id,$val,$rank) = @{$_};
			$table .= sprintf('<tr><td>Tag: %s</td><td>%s</td></tr>', $cv{$id}, $val);
		}
